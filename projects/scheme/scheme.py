...
from argparse import ArgumentError  # Python 2 compatibility
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('The number of formals does not equal to the number of vals')
        child = Frame(self)
        p, q = formals, vals
        while p is not nil:
            child.define(p.first, q.first)
            p, q = p.rest, q.rest
        return child
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('arguments number does not fit')
        # END PROBLEM 3
...
class MacroProcedure(LambdaProcedure):
    """A macro: a special form that operatretun es on its unevaluated operands to
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a dynamic-scoped call evaluated in my parent environment."""
