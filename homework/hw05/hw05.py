...
# @Date:   2021-01-21 16:21:19
# @Last Modified by:   Xia Hanyu (Humprey Chou)
# @Last Modified time: 2021-01-21 17:30:15
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, passwd):
        ...
        if len(lst) == 3:
            return 'Too many incorrect attempts. Attempts: {0}'.format(lst)
        if passwd != password:
            lst.append(passwd)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    access = withdraw(0, old_pass)
    if type(access) == str:
        return access
    def new_access(amount, passwd):
        if passwd in [old_pass, new_pass]:
            passwd = old_pass
...
def naturals():
    ...
    ATTENTION: Natural numbers includes 0, thus I added it
    >>> m = naturals()
    >>> type(m)
    <class 'generator'>
