...
def in_order_traversal(t):
    ...
    def generate(t):
        if t.is_leaf():
            ...
            generate(t.branches[0])
            seq.append(t.label)
            generate(t.branches[1])
    generate(t)
    yield from seq
...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    def solve(cur, odd_term, even_term, flag):
        if cur == n + 1:
            return 0
        return (odd_term(cur) if flag else even_term(cur)) + solve(cur + 1, odd_term, even_term, not flag)
    return solve(1, odd_term, even_term, True)
