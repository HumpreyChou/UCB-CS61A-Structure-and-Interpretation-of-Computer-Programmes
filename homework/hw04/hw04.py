...
# @Date:   2021-01-20 21:25:31
# @Last Modified by:   Xia Hanyu (Humprey Chou)
# @Last Modified time: 2021-01-20 22:32:55
...
def quadratic(x, a, b, c):
    ...
    pivot = -b / (2 * a)
    if a > 0:
        if pivot > upper_bound(x):
            return interval(f(upper_bound(x)), f(lower_bound(x)))
        elif pivot < lower_bound(x):
            return interval(f(lower_bound(x)), f(upper_bound(x)))
        else:
            if pivot < (lower_bound(x) + upper_bound(x)) / 2:
                return interval(f(pivot), f(upper_bound(x)))
            else:
                return interval(f(pivot), f(lower_bound(x)))
    elif a < 0:
        if pivot > upper_bound(x):
            return interval(f(lower_bound(x)), f(upper_bound(x)))
        elif pivot < lower_bound(x):
            ...
            if pivot < (lower_bound(x) + upper_bound(x)) / 2:
                return interval(f(upper_bound(x)), f(pivot))
            else:
                return interval(f(lower_bound(x)), f(pivot))
    else: # a == 0
