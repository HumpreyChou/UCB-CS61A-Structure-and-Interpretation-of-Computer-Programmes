...
# @Date:   2021-01-21 12:02:09
# @Last Modified by:   Xia Hanyu (Humprey Chou)
# @Last Modified time: 2021-01-21 15:26:44
...
def subseqs(s):
    ...
    if not s:
        ...
        except_first = subseqs(s[1:])
        return except_first + insert_into_all(s[0], except_first)
...
def make_glookup(class_assignments):
    ...
    def calculate(assignment, score):
        nonlocal total_score, total_assign_score, assignments
        if assignments not in assignments:
            assignments.add(assignment)
            total_assign_score += class_assignments[assignment]
        total_score += score
        return total_score / total_assign_score
    return calculate
