...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char != code_char:
            ...
            possibilities.append(("+[{0}]".format(code_char) + edits, cost + 1))
        # Delete
            edits, cost = helper_align(skeleton_idx + 1, code_idx)
            possibilities.append(("-[{0}]".format(skel_char) + edits, cost + 1))
    ...
    def solve(sum1, sum2, idx):
        ...
        if idx == len(s):
            if abs(sum1 - sum2) <= d:
                ans += 1
            return
        solve(sum1 + s[idx], sum2, idx + 1)
        solve(sum1, sum2 + s[idx], idx + 1)
    solve(0, 0, 0)
    return ans // 2
